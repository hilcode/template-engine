...
public final class TransitionParametersVariableDelimitersSetting
	implements
		StateMachine.Transition<Line, Mode, TengParserState>
{
	private static final Pattern VARIABLE_DELIMITERS_SETTING_LINE;
	static
	{
		final String delimiterChars = "[\\p{Punct}[a-zA-Z0-9]]";
		VARIABLE_DELIMITERS_SETTING_LINE = Pattern.compile("^variable-delimiters[ \t]+(" + delimiterChars + "+)[ \t]+(" + delimiterChars + "+)$");
	}

	@Override
	public ModeAndState<Mode, TengParserState> apply(final Mode mode, final TengParserState state, final Line line)
	{
		final Matcher matcher = VARIABLE_DELIMITERS_SETTING_LINE.matcher(line.text);
		if (matcher.matches())
		{
			state.variableBegin = matcher.group(1);
			state.variableEnd = matcher.group(2);
