...
public final class TransitionParametersDirectivePrefixSetting
	implements
		StateMachine.Transition<Line, Mode, TengParserState>
{
	private static final Pattern DIRECTIVE_PREFIX_SETTING_LINE;
	static
	{
		final String delimiterChars = "[\\p{Punct}[a-zA-Z0-9]]";
		DIRECTIVE_PREFIX_SETTING_LINE = Pattern.compile("^directive-prefix[ \t]+(" + delimiterChars + "+)$");
	}

	@Override
	public ModeAndState<Mode, TengParserState> apply(final Mode mode, final TengParserState state, final Line line)
	{
		final Matcher matcher = DIRECTIVE_PREFIX_SETTING_LINE.matcher(line.text);
		if (matcher.matches())
		{
			state.directivePrefix = matcher.group(1);
