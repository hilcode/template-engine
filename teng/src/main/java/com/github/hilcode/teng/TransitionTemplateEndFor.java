...
public final class TransitionTemplateEndFor
	implements
		StateMachine.Transition<Line, Mode, TengParserState>
{
	...
	public ModeAndState<Mode, TengParserState> apply(final Mode mode, final TengParserState state, final Line line)
	{
		...
		if (trimmedLine.equals(state.directivePrefix + "end-for"))
		{
			state.indentationLevel--;
			state.templateBuilder.add(lineAndSource(repeat("    ", state.indentationLevel + 4) + "}", templateLine(line.number, line.text)));
